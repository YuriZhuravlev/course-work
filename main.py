...
def f(x, y, t):
    return np.sin(np.pi / 2 * x) * np.sin(np.pi / 2 * y) * np.cos(3 * t) * (-9 + np.pi ** 2 / 2)
...
def phi(x, y):
    return np.sin(np.pi / 2 * x) * np.sin(np.pi / 2 * y)
...
if __name__ == '__main__':
    print("Запуск")
    # task(14, _ht=0.1, f=f, phi=phi, ksi=ksi, filename='resources/Пример 0', _a=2, _b=2)
    check_error(14, _ht=0.1, f=f, phi=phi, ksi=ksi, filename='resources/error0', _a=2, _b=2)